...
export const NavItems = ({ items, onItemClick, className }: NavItemsProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        'absolute inset-0 hidden flex-1 flex-row items-center justify-center space-x-2 text-sm font-medium text-zinc-600 transition hover:text-zinc-800 lg:flex',
        className
      )}
    >
      {items.map((item, idx) => (
...
export const NavbarButton = ({
  ...
  const base =
    'px-4 py-2 rounded-md text-sm font-bold inline-block text-center transition duration-200';
